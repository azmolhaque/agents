...
def test_a_separated_local_part_is_a_name_and_a_bare_one_is_a_guess():
    """`full_name` had one reader and zero writers.

    `worklist` read it, `has_named_contact` was worth +10 of the reachability
    component, `persona_for` routed on it, and `_recipient_name` fed it to the outreach
    prompt as `recipient` -- and nothing in the codebase ever assigned a value.
    Measured 2026-09-04: **289 contacts, 0 names.** Same shape as `digest_pages`,
    `extend_lease` and `open_roles`: built, wired at both ends, never connected.

    Only the separated forms count. `sarah.chen@` is structurally a name -- the company
    chose to publish first-and-last with a separator. `jdoe@` is an initial and a
    surname, or a nickname, or neither, and "Hi Jdoe" is worse than no greeting in
    exactly the way "Hi Support" is. Act where the evidence has structure; infer nothing
    where it does not.
    """
    from cindraleads.contacts import name_from_local_part

    assert name_from_local_part("sarah.chen@acme.io") == "Sarah Chen"
    assert name_from_local_part("sarah-chen@acme.io") == "Sarah Chen"
    assert name_from_local_part("md_azmol@acme.io") == "Md Azmol"

    assert name_from_local_part("souren@acme.io") is None, "a bare token is a guess"
    assert name_from_local_part("jdoe@acme.io") is None
    assert name_from_local_part("a.b@acme.io") is None, "initials are not a name"
    assert name_from_local_part("user123.x@acme.io") is None, "digits are not a name"

    # A role mailbox is not a person however it is punctuated.
    assert name_from_local_part("info@acme.io") is None
    assert name_from_local_part("no-reply@acme.io") is None


async def test_a_named_address_reaches_the_contacts_table(tmp_path: Path):
    """The field crosses `extract_contacts` -> `EnrichOutcome` -> the INSERT, and this
    project has shipped three defects that lived exactly in that seam with tests
    passing on both ends of it. So this drives the real Enricher and reads the row."""
    store = Store(tmp_path / "named.db", migrations_dir=MIGRATIONS)
    store.migrate()
    with store.tx() as conn:
        conn.execute(
            "INSERT INTO companies (canonical_domain, display_name, ai_surface, tech_signals, "
    ...
    page = '<html><body>Write to <a href="mailto:sarah.chen@acme.io">Sarah</a></body></html>'
    ...
    enricher = Enricher(
        store=store,
        egress=egress,
        dns=StubProbe({("acme.io", "MX"): ["10 mx.acme.io."]}),
        enabled_sources=frozenset({"site", "dns"}),
    )

    assert (await enricher.run(job())).ok
    row = store.conn.execute("SELECT full_name, role_title, persona FROM contacts").fetchone()
    assert row["full_name"] == "Sarah Chen"
    # And `persona` stays None, which is the design rather than a gap: `persona_for`
    # reads a *role title*, and knowing someone is called Sarah Chen says nothing about
    # whether she is the CTO. Role titles need the team page this does not read.
